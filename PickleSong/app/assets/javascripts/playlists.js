$(document).ready(function() {
    $('.playlist').click(function() {
        ...
        var playlist = $(this);
        var titleTemplate = "<h4 class='active-playlist' data-id='<%= item.data('playlistid') %>'><%= item.data('name') %></h4><button class='delete-button'>Delete</button>";
        ...
        $('.delete-button').click(function() {
            $.ajax({
                url: '/playlists/' + $(this).data("playlistid"),
                type: 'DELETE'
            });
        });
